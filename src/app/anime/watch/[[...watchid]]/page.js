...
import { createWatchEp, getEpisode, getAnimeWatchHistory } from "@/lib/EpHistoryfunctions";
...
async function AnimeWatch({ params, searchParams }) {
  ...
  const aniListInfo = await WatchPageInfo(session?.user?.token, id);
  // console.log(savedep)
  // console.log(data)
    ...
    <>
      ...
      <div className=" w-full flex flex-col lg:flex-row lg:max-w-[98%] mx-auto xl:max-w-[94%] lg:gap-[6px] mt-[70px]">
        <div className="flex-grow w-full h-full">
          <PlayerComponent id={id} epId={epId} provider={provider} epNum={epNum} data={data} subdub={subdub} session={session} savedep={savedep} aniListInfo={aniListInfo}/>
