...
function UserWatchingList({ data }) {
    ...
    const smoothScroll = (amount) => {
        const container = containerRef.current;
        const cont = document.getElementById('Continue Watching');
    ...
    return (
        <div className={styles.animecard}>
            ...
            <div className={styles.animeitems}>
                ...
                <div className={styles.cardcontainer} id="Continue Watching" {...events} ref={containerRef} onScroll={handleScroll}>
                    {data.map((item) => {
                        const anime = {
                            id: item.id || '',
                        ...
                        return (
                            <Link href={`/anime/info/${anime.id}`} key={`${anime.id}-${item.listId}`}>
                                <ItemContent anime={anime} cardid="Continue Watching" />
                            </Link>
                        );
                    })}
                </div>
