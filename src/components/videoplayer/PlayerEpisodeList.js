...
function PlayerEpisodeList({ id, data, onprovider, initialProvider, setwatchepdata, epnum, progress }) {
  ...
  useEffect(() => {
    ...
    // Pass filtered episodes with provider metadata attached
    if (filteredEp && Array.isArray(filteredEp)) {
      const episodesWithMeta = filteredEp.map(ep => ({
        ...ep,
        _providerData: provider, // Attach full provider data to each episode
      }));
      setwatchepdata(episodesWithMeta);
      setCurrentEpisodes(episodesWithMeta);
    } else {
              ...
              <>
                <div className={styles.episodetitle}>
                  ...
                  <div className={styles.epright}>
                    ...
                    >
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke={`${eplisttype === 2 ? '#a78bfa' : 'currentColor'}`} className="w-6 h-6">
                    ...
                    >
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke={`${eplisttype === 3 ? '#a78bfa' : 'currentColor'}`} className="w-6 h-6">
                ...
                {eplisttype === 2 && (
                  <div className="mt-3">
                    <EpImgContent data={data} epdata={filteredEp} defaultProvider={defaultProvider} subtype={subtype} epnum={epnum} progress={progress}/>
                  </div>
                )}
                {eplisttype === 3 && (
                  <div className={styles.epnumlist}>
                    <EpNumList data={data} epdata={filteredEp} defaultProvider={defaultProvider} subtype={subtype} epnum={epnum} progress={progress}/>
                  </div>
                )}
              </>
            )}
