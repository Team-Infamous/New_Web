...
function PlayerComponent({ id, epId, provider, epNum, subdub, data, session, savedep, aniListInfo }) {
        ...
        const fetchSources = async () => {
            ...
            try {
                ...
                const reFormSubtitles = subtitlesArray?.map((i) => ({
                    ...
                    kind: i?.kind || (i?.lang?.toLowerCase().includes("thumbnail") ? "thumbnails" : "subtitles"),
        ...
        <div className='xl:w-[99%]'>
            ...
            <div className='w-[98%] mx-auto lg:w-full'>
                <PlayerEpisodeList 
                    ...
                    epnum={epNum} 
                    progress={aniListInfo?.mediaListEntry?.progress}
                />
            </div>
        </div>
    )
