  ...
  return (
    ...
    <Tooltip.Root>
      ...
      <Menu.Content className={menuClass} placement={placement}>
        ...
        {  subtitles?.length > 0 && <SubtitleSizeSubmenu/>}
        <QualitySubmenu />
        </Menu.Content>
      {/* </Menu.Portal> */}
  </Menu.Root>
...
function SubtitleSizeSubmenu() {
  const settings = useStore(useSettings, (state) => state.settings);
  const currentSize = settings?.subtitleSize || "100";
  const options = [
    { label: "50%", value: "50" },
    { label: "75%", value: "75" },
    { label: "100%", value: "100" },
        ...
        >
          {options.map(({ label, value }) => (
            <Radio 
                ...
                key={value} 
                onSelect={() => {
                    useSettings.setState({ settings: { ...useSettings.getState().settings, subtitleSize: value } })
                }}
            >
              {label}
            </Radio>
