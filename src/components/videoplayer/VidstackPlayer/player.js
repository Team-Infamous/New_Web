...
function Player({ dataInfo, id, groupedEp, src, session, savedep, subtitles, thumbnails, skiptimes }) {
  ...
  function onEnded() {
    if (session && !progressSaved) {
      try {
        setprogressSaved(true);
        saveProgress(session.user.token, dataInfo?.id || id, Number(epNum) || Number(currentep?.number), dataInfo?.episodes);
      } catch (error) {
  ...
  function onTimeUpdate() {
    ...
    if (session && !progressSaved && percentage >= 0.7) {
      try {
  ...
  return (
    <MediaPlayer key={src} ref={playerRef} playsInline aspectRatio={16 / 9} load={settings?.load || 'idle'} muted={settings?.audio || false}
      ...
      style={{
        "--subtitle-scale": (Number(settings?.subtitleSize) || 100) / 100
      }}
    ...
    >
      ...
      <VideoLayout
        subtitles={subtitles}
        thumbnails={thumbnails && process.env.NEXT_PUBLIC_PROXY_URI ? process.env.NEXT_PUBLIC_PROXY_URI + '/' + thumbnails[0]?.src : ""}
        groupedEp={groupedEp}
      />
