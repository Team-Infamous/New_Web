...
export async function fetchKaidoEpisodes(animeId) {
  try {
    ...
    const hasSplit = data && !Array.isArray(data) && (Array.isArray(data.sub) || Array.isArray(data.dub));
    if (hasSplit) {
      const subEpisodes = Array.isArray(data.sub) ? data.sub.map(mapEpisode) : [];
      const dubEpisodes = Array.isArray(data.dub) ? data.dub.map(mapEpisode) : [];
      logger.log(`[Kaido] Successfully fetched ${subEpisodes.length} sub and ${dubEpisodes.length} dub episodes`);
...
export async function fetchHiAnimeEpisodes(animeId) {
  try {
    ...
    if (hasSplit) {
      const subEpisodes = Array.isArray(data.sub) ? data.sub.map(mapEpisode) : [];
      const dubEpisodes = Array.isArray(data.dub) ? data.dub.map(mapEpisode) : [];
      logger.log(`[HiAnime] Successfully fetched ${subEpisodes.length} sub and ${dubEpisodes.length} dub episodes`);
      return { sub: subEpisodes, dub: dubEpisodes };
    }
...
const fetchAndCacheData = async (id, meta, redis, cacheTime, refresh, genres = [], title = '', fast = false) => {
  ...
  if (isHentai) {
    ...
    const isSplitEpisodes = (episodes) =>
      episodes && !Array.isArray(episodes) && (Array.isArray(episodes.sub) || Array.isArray(episodes.dub));

    results.forEach(result => {
      const split = isSplitEpisodes(result.episodes);
      const subCount = split ? (result.episodes.sub?.length || 0) : 0;
      const dubCount = split ? (result.episodes.dub?.length || 0) : 0;
      const arrayCount = Array.isArray(result.episodes) ? result.episodes.length : 0;
      const hasEpisodes = arrayCount > 0 || subCount > 0 || dubCount > 0;
      ...
      if (hasEpisodes) {
        if (result.type === 'kaido') {
          ...
          logger.log(`[EPISODES] Added Kaido with ${split ? `${subCount} sub / ${dubCount} dub` : `${arrayCount}`} episodes`);
          ...
          logger.log(`[EPISODES] Added HiAnime with ${split ? `${subCount} sub / ${dubCount} dub` : `${arrayCount}`} episodes`);
          ...
          logger.log(`[EPISODES] Added AnimePahe with ${arrayCount} episodes`);
        }
      }
    });
